...
public class GroupUtil {
	...
	/*public static List<ArticleGroup> prebuild666(HibernateTemplate ht,List<ArticleGroup> list,ArticleGroup disabler,String pre,String preFront,String preEnd){
		...
		for (ArticleGroup g:list) {
			...
			if (disabler == null || (disabler != null && !(g.getFootLeft()>=disabler.getFootLeft() && g.getFootRight() <= disabler.getFootRight()))) {
				...
				String preStr;
				if (preEnd==null || preEnd.trim().equals("")) {
					preEnd = pre;
				}
				if (rup.getNextID()==0) {
					preStr=preEnd;
				...
				g.setTitle(StringUtil.countStr((g.getOrderStr().length()-1)/3 - 1,preFront)+preStr+g.getName());
